...
on<ImportNameChanged>((e, emit) {
  emit(state.copyWith(nameValue: e.value, error: null)); // clear stale error
});
  ...
  Future<void> _onSubmit(ImportSubmitPressed event, Emitter<ImportState> emit) async {
    ...
    // 1) empty
    if (mat.isEmpty) {
      emit(state.copyWith(error: 'Please enter Material No.'));
      return;
    }

    // ตัวอย่างการใช้ใน validate
    if (!RegExp(r'^\d{8}$').hasMatch(mat)) {
      _emitSnack(emit, 'Material No. must be exactly 8 digits.', error: true);
      return;
    }
    ...
    if (!resp.success) {
      emit(state.copyWith(isAdding: false, error: resp.error ?? 'Failed to submit data.'));
      return;
    }

    final newMatLength = resp.data.length; // (fix typo: Length)
    final String msg = switch (newMatLength) {
      0 => 'No new records were added.',
      1 => 'Added 1 new record.',
      _ => 'Added $newMatLength new records.',
    };
  ...
  Future<void> _onStart(ImportStartPressed event, Emitter<ImportState> emit) async {
    ...
    if (!resp.success || resp.data.isEmpty) {
      emit(state.copyWith(isWaiting: false, error: resp.error ?? 'Unable to start processing.'));
      return;
    }
  ...
  void _startPolling() {
    ...
    _pollSub = Stream<void>.periodic(_interval).asyncMap((_) => _apis.progress()).listen((resp) {
      if (!resp.success || resp.data.isEmpty) {
        add(_ImportProgressFailed(resp.error ?? 'Unable to fetch progress.'));
        return;
      }
      try {
        add(_ImportProgressArrived(DataImporting.fromJson(resp.data.first)));
      } catch (e) {
        add(_ImportProgressFailed('Invalid progress data structure: $e'));
      }
    }, onError: (e, __) {
    ...
    () async {
      ...
      if (!first.success || first.data.isEmpty) {
        add(_ImportProgressFailed(first.error ?? 'Unable to fetch progress.'));
        return;
      }
      try {
